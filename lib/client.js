...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function SubagentsTab(props) {
      ...
      // Keep the card grid pinned to the top when this tab is switched to /
      // first opened. The conversation.view ring unmounts inactive tabs, so
      // this component remounts on every activation and this mount-only effect
      // runs right at that point (before first paint). An empty dependency
      // array means it never re-runs afterwards, so the user's own scrolling
      ...
      return React.createElement("div", { ref: gridRef, style: { padding: 16, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: 8, overflow: "auto", height: "100%", alignContent: "start" } },
